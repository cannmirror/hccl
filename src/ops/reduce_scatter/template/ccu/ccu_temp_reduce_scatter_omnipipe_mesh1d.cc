    ...
    if(topoInfo->level0Topo != Level0Shape::MESH_1D_CLOS) {
        ...
        for(auto channel : channelDescs){
            if(channel.channelProtocol != COMM_PROTOCOL_UBC_CTP){
                HCCL_ERROR("[CcuTempReduceScatterOmniPipeMesh1D][CalcRes] channelProtocol: %u", channel.channelProtocol);
    ...
    const OpParam &param, const TemplateDataParams &templateDataParams, TemplateResource &templateResource)
{
    ...
    if (localCopyFlag == 0) {
        ...
        for (uint32_t rpt = 0; rpt < repeatNum; ++rpt) {
            ...
            std::vector<uint64_t> taskArgs = {inputAddr, outputAddr, scratchAddr, sliceSize, offset, token, localCopyFlag,
            ...
            CHK_PRT_RET(launchRet != CCU_SUCCESS, 
                HCCL_ERROR("[CcuTempReduceScatterOmniPipeMesh1D::KernelRun] kernel launch failed, ccuRet -> %d", launchRet), ConvertCcuToHccl(launchRet));
        }
