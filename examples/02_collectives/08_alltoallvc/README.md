...
本样例展示如何使用 `HcclAlltoAllVC()` 接口执行 AlltoAllVC 操作，包含以下功能点：
...
- 调用 `HcclAlltoAllVC()` 算子，将输入数据在特定的维度切分成特定的块数，并按顺序发送给其他 rank，同时从其他 rank 接收数据，按顺序在特定的维度拼接数据，最后打印结果。
